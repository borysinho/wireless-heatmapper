...
La base de datos está organizada alrededor de `proyecto`. Un administrador gestiona `usuario` y `cliente`; el técnico trabaja sobre `plano`, captura `punto_medicion` y `medicion_wifi`, define `conjunto_ap` con los APs relevantes del proyecto y genera `mapa_calor`. El portal cliente se controla con `token_enlace_cliente`, cuyo contenido selecciona explícitamente conjuntos y mapas.
...
- El heatmap proyectado se guarda como `mapa_calor` asociado al conjunto IA.
...
package "Inventario RF físico" {
  ...
  entity "radio_ap" as RADIO {
    * id : INTEGER <<PK>>
    ...
    canal : INTEGER
    ancho_canal_mhz : INTEGER
    potencia_tx_dbm : FLOAT
    patron_antena : VARCHAR(80)
  ...
  entity "bssid_radio" as BSSID_RADIO {
    * id : INTEGER <<PK>>
    --
    radio_ap_id : INTEGER <<FK, NOT NULL>>
...
PLANO ||--o{ AP_FISICO : "inventario"
AP_FISICO ||--o{ RADIO : "radios"
RADIO ||--o{ BSSID_RADIO : "BSSID"
...
| `medicion_wifi`          | `UNIQUE(punto_id, bssid, numero_lectura)`                 | Evita lecturas duplicadas del mismo BSSID           |
...
| `bssid_radio`            | `UNIQUE(bssid)`                                           | Un BSSID físico se registra una sola vez            |
...
| `plano` → `ap_fisico`            | `ON DELETE CASCADE`            | Borrar plano elimina inventario RF                      |
| `ap_fisico` → `radio_ap`         | `ON DELETE CASCADE`            | Borrar AP físico elimina radios                         |
| `radio_ap` → `bssid_radio`       | `ON DELETE CASCADE`            | Borrar radio elimina BSSID asociados                    |
...
2. El backend persiste `punto_medicion` y `medicion_wifi`.
...
7. Cada propuesta IA puede tener igual, menor o mayor cantidad de items que el conjunto fuente.
8. El mapa proyectado de cada propuesta se persiste como `mapa_calor` asociado al conjunto IA.
...
| Captura en campo          | `plano`, `punto_medicion`, `medicion_wifi`                 | Observaciones reales del relevamiento WiFi          |
...
| Inventario RF             | `ap_fisico`, `radio_ap`, `bssid_radio`                     | Modelo físico RF para calibración y optimización    |
