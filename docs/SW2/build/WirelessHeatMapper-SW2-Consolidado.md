...
**Grupo:** 24
**Modalidad oficial:** 100 % en línea
**Cliente del caso:** Bulldog Tech.
**Fuente base:** [PAPS Online](../Wireless%20Heatmapper%20-%20PAPS%20-%20Modalidad%20Online.md)

---

### Propósito del documento

Este documento resume el **Plan Aplicado a Proyecto de Software (PAPS)** en una versión operativa para SW2. No reemplaza al PAPS Online ni duplica sus métricas extensas; concentra la definición del problema, la situación deseada, el alcance, las restricciones, los requerimientos principales y el plan de entrega por sprints. Los detalles metodológicos, de backlog y trazabilidad se consultan en:

- [01 — Marco Scrum Online](01-marco-scrum-online.md)
- [05 — Product Backlog Online](05-product-backlog-online.md)
- [06 — Plan de Sprints](06-plan-de-sprints.md)
- [14 — Trazabilidad RP ↔ HU](14-trazabilidad-rp-hu.md)

---

### Problema

El diseño de redes WiFi interiores suele basarse en estimaciones, experiencia del técnico y revisiones tardías después de instalar los puntos de acceso. Esto provoca zonas con señal débil, interferencias, solapamientos innecesarios y retrabajo cuando el cliente detecta fallas de cobertura en operación real.

El problema se agrava porque las mediciones de campo, los planos, los mapas de calor y las decisiones técnicas no siempre quedan integrados en una fuente central consultable por técnicos, administradores y clientes. En consecuencia, Bulldog Tech. necesita un flujo trazable para levantar evidencia RF, procesarla y compartir resultados sin depender de archivos locales dispersos ni de sincronización manual.

---

### Situación deseada

Se espera contar con un sistema integrado y estrictamente en línea donde:

- El técnico registre proyectos, planos, puntos de medición y lecturas WiFi desde una app Android.
- Cada operación de dominio se persista en el backend central mediante API REST.
- El backend genere mapas de calor y mantenga la evidencia RF en PostgreSQL.
- El módulo de IA proponga escenarios de mejora comparables contra la situación medida.
- El administrador gestione usuarios, clientes y proyectos desde una plataforma web.
- El cliente acceda por enlace único a los resultados publicados, sin instalar la app móvil.

La situación deseada prioriza trazabilidad, disponibilidad centralizada y validación visual del estado actual y proyectado de la cobertura WiFi.
...
Desarrollar un sistema online para capturar, procesar, visualizar y optimizar cobertura WiFi en interiores mediante mapas de calor, administración web, backend REST centralizado e IA de apoyo a la ubicación de puntos de acceso.

#### Objetivos específicos

- Permitir la gestión de proyectos de survey asociados a clientes y técnicos.
- Importar y calibrar planos para ubicar mediciones en coordenadas útiles.
- Capturar lecturas WiFi y enviarlas al backend en línea.
- Generar mapas de calor a partir de mediciones reales y conjuntos de APs.
- Proponer escenarios de optimización RF como conjuntos derivados por IA.
- Comparar el escenario medido con el escenario propuesto sin alterar la evidencia original.
- Publicar resultados seleccionados en un portal web accesible por enlace único.
- Mantener administración centralizada de usuarios, clientes y proyectos.
...
| Área             | Alcance operativo                                                                 |
| ---------------- | --------------------------------------------------------------------------------- |
| App móvil        | Login, proyectos, planos, calibración, captura WiFi, puntos y visualización base  |
| Backend REST     | Autenticación, dominio central, persistencia, heatmaps, IA y publicación          |
| Base de datos    | PostgreSQL como única fuente de verdad del dominio                                |
| Web admin        | Gestión de usuarios, clientes y supervisión de proyectos                          |
| Portal cliente   | Consulta por enlace único de heatmaps, análisis y plan AP publicado               |
| IA / RF          | Escenarios derivados, comparación y apoyo a ubicación/configuración de APs        |

#### Fuera de alcance vigente

| Elemento descartado                | Motivo                                                                 |
| ---------------------------------- | ---------------------------------------------------------------------- |
| Sincronización offline (`PB-14`)   | La modalidad oficial es 100 % en línea                                 |
| Persistencia local de dominio      | PostgreSQL es la única fuente de verdad                                |
| Diagnóstico persistido separado    | Las métricas se derivan de mediciones, conjuntos AP y mapas de calor   |
| Exportación PDF (`PB-08`)          | El portal cliente reemplaza el PDF como mecanismo principal de entrega |
...
| Tipo             | Restricción                                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------- |
| Modalidad        | El sistema opera 100 % en línea; no hay sincronización diferida ni base local de dominio en móvil |
| Persistencia     | Toda entidad de negocio se persiste en PostgreSQL mediante el backend FastAPI                     |
| Android          | En Android ≥ 8.0 aplica throttling de escaneo: 4 escaneos cada 2 minutos                          |
| Cobertura RF     | RSSI < −90 dBm = zona muerta; objetivo de diseño ≥ −70 dBm                                       |
| Arquitectura     | Cliente móvil delgado, API REST, panel web y portal cliente conectados al backend                 |
| Tecnología       | Usar el stack aprobado; no introducir frameworks alternativos sin acuerdo del PO                  |
| Trazabilidad     | Mantener relación entre `RP`, `PB`, sprint y `UC` en la matriz vigente                            |
| Documentación    | Markdown y PlantUML; preservar identificadores estables del plan                                  |

---

### Requerimientos principales

| RP  | Requerimiento principal                                      | HU asociadas          | Sprint | Estado de alcance                         |
| --- | ------------------------------------------------------------ | --------------------- | ------ | ----------------------------------------- |
| RP1 | Captura de señal WiFi en línea                               | PB-03                 | S3     | Activo                                    |
| RP2 | Mapeo sobre plano: importar, calibrar y marcar puntos        | PB-02, PB-11, PB-04   | S2-S3  | Activo                                    |
| RP3 | Generación de mapas de calor en backend                      | PB-20, PB-05          | S4     | Activo                                    |
| RP4 | Análisis automático de cobertura como diagnóstico persistido | —                     | —      | Fuera de alcance vigente                  |
| RP5 | Optimización mediante IA como conjuntos AP derivados         | PB-07, PB-12          | S5     | Activo                                    |
| RP6 | Generación/exportación de reportes PDF                       | —                     | —      | Reemplazado por portal cliente            |
| RP7 | Administración de usuarios, clientes y supervisión           | PB-13, PB-19, PB-18   | S1     | Activo                                    |
| RP8 | Persistencia centralizada en línea                           | PB-09, PB-01, PB-10   | S1     | Activo y transversal                      |
| RP9 | Portal de cliente por enlace único                           | PB-15, PB-16, PB-17   | S6     | Activo                                    |

La trazabilidad completa se mantiene en [14-trazabilidad-rp-hu.md](14-trazabilidad-rp-hu.md).

---

### Stack tecnológico

| Componente            | Tecnología aprobada                                      |
| --------------------- | -------------------------------------------------------- |
| App móvil             | Flutter / Dart con BLoC/Cubit                            |
| Backend REST + IA     | Python / FastAPI                                         |
| Base de datos         | PostgreSQL 15+                                           |
| Web admin y portal    | React + TypeScript + Vite                                |
| Infraestructura       | Docker Compose + Nginx                                   |
| CI/CD                 | GitHub Actions                                           |
| Modelado              | StarUML y PlantUML compatible con UML 2.5+               |

---

### Cronograma por sprints

| Sprint   | Fechas planificadas        | HU principales                           | Objetivo de entrega                                      | Criterio de aceptación macro                                      |
| -------- | -------------------------- | ---------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------ |
| S0       | 13-17 abr 2026             | —                                        | Base técnica, contenedores, CI/CD y modelos iniciales    | Backend responde healthcheck y entorno levanta con Docker Compose  |
| S1       | 20-26 abr 2026             | PB-13, PB-19, PB-09, PB-18, PB-01, PB-10 | Fundación CRUD, admin web, autenticación y proyectos     | Admin crea usuarios/clientes; técnico inicia sesión y gestiona proyectos |
| S2       | 28 abr-11 may 2026         | PB-02, PB-11                             | Importación y calibración de planos en línea             | Plano queda asociado al proyecto y calibrado con escala real       |
| S3       | 12-25 may 2026             | PB-03, PB-04                             | Captura WiFi con ingesta REST y puntos de medición       | Lecturas capturadas desde la app aparecen persistidas en PostgreSQL |
| S4       | 26 may-8 jun 2026          | PB-20, PB-05                             | Conjuntos AP y mapas de calor backend                    | Heatmap se genera por conjunto/AP y se visualiza sobre el plano    |
| S5       | 9-22 jun 2026              | PB-07, PB-12                             | IA y comparación de escenarios                           | Propuesta IA queda como conjunto derivado y puede compararse       |
| S6       | 23 jun-6 jul 2026          | PB-15, PB-16, PB-17                      | Portal cliente con enlace único                          | Cliente abre un enlace válido y ve los resultados publicados       |
| Cierre   | 7-11 jul 2026              | Validación integrada                     | Pruebas finales, documentación y entrega                 | Evidencia final, APK, URLs y documentación quedan disponibles      |

---

### Criterios de aceptación generales

Un incremento se acepta cuando cumple las siguientes condiciones:

- La HU implementada satisface sus criterios funcionales y el flujo se puede demostrar en R-4.
- Los datos de dominio se guardan en PostgreSQL mediante el backend, sin persistencia local móvil.
- Los endpoints relevantes validan autenticación, permisos y pertenencia organizacional.
- La interfaz móvil o web muestra estados de carga, error y éxito suficientes para operar el flujo.
- Las restricciones RF aplicables se respetan en captura, heatmaps, comparación o IA.
- La implementación mantiene trazabilidad `RP ↔ HU ↔ Sprint ↔ UC`.
- Las pruebas razonables del cambio quedan ejecutadas o documentadas si no aplican.
- La documentación del sprint o del artefacto afectado queda actualizada cuando cambia el alcance.

---

### Criterios de cierre del PAPS SW2

El PAPS adaptado se considera vigente mientras:

- El PAPS Online siga siendo la fuente oficial de alcance.
- El plan Scrum mantenga el backlog, los sprints y la matriz de trazabilidad sincronizados.
- Las decisiones de refinamiento se documenten sin reintroducir sincronización offline, PDF obligatorio ni base local de dominio.
- Bulldog Tech. pueda revisar resultados en línea mediante el portal cliente y evidencia técnica trazable.
